...
class HyperactiveResource < ActiveResource::Base
  ...
  #  But future changes aren't kept in sync (like ActiveRecord.. mostly)
    ...
    # conditions.
    # Note - this code assumes that you will implement the API in a railsish
    # way, ie that the API will respond to a URL that would match that
    # generated by a RESTful URL of the form: count_widgets_path
    # If you pass in conditions - it also assume that your API will only
    # send items that match conditions when passed it the form of query
    # string parameters.
    # If neither of these are the case, the alternative code is included and
    # you can simply uncomment it here to make it work...
    # ...but if this is the case - it actually has to do a full fetch and
